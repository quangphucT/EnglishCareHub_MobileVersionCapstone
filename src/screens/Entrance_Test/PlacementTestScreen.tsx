...
import { useGetPlacementTest, useSubmitPlacementTest } from '../../hooks/learner/placementTest/placementTestHooks';
...
export default function PlacementTestScreen() {
  ...
  const submitMutation = useSubmitPlacementTest();
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status !== 'granted') {
        Alert.alert('Quyền microphone', 'Vui lòng cấp quyền microphone để sử dụng tính năng này.');
      }
    })();
  }, []);

  const convertBlobToBase64 = useCallback(async (blob: Blob): Promise<string> => {
...
const handleLogout = async () => {
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      ...
      const tests = Array.from(sectionMap.entries()).map(([type, questions]) => ({
        type,
  ...
  if (isLoading) {
    return (
      <View className="flex-1 bg-white items-center justify-center">
        ...
        <Text className="mt-4 text-gray-600 text-base">Đang tải bài test...</Text>
      </View>
    );
  }

  if (!testData?.data || !currentQuestion) {
    return (
      <View className="flex-1 bg-white items-center justify-center px-6">
        <Ionicons name="alert-circle-outline" size={64} color="#EF4444" />
        <Text className="mt-4 text-gray-800 text-xl font-bold">Không có câu hỏi</Text>
        <TouchableOpacity
     ...
     <View className="flex-1 bg-gray-50">
      ...
      >
          ...
          <View className="flex-row items-center justify-between mb-4">
            <View>
              <Text className="text-white text-2xl font-bold">Placement Test</Text>
              <Text className="text-white/90 text-sm mt-1">{currentQuestion.sectionType.toUpperCase()}</Text>
            </View>
        ...
        <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
          ...
          <View className="mt-6 bg-slate-50 rounded-3xl p-6 border-2 border-gray-200">
            <View className="items-center mb-4">
              ...
              <View className="mt-2 bg-blue-50 px-3 py-1 rounded-full">
                <Text className="text-xs font-semibold text-blue-700">
                  Còn {2 - (recordingAttempts[currentQuestionIndex] || 0)}/2 lượt
                </Text>
              </View>
            </View>
            ...
            <TouchableOpacity
              ...
              style={{ 
                opacity: (recordingAttempts[currentQuestionIndex] || 0) >= 2 && recordingStatus !== 'recording' ? 0.5 : 1 
              }}
            >
              <View
                style={{ 
          ...
          {recorded[currentQuestionIndex] && (
            ...
            >
              <Text className="text-white font-bold text-base">
                {isPlayingAudio ? '▶ Đang phát...' : '▶ Nghe lại bản ghi âm'}
              </Text>
            </TouchableOpacity>
          )}
        </ScrollView>
          ...
          <View className="bg-white rounded-3xl p-8 border border-gray-200">
            {isSubmitting ? (
              <View className="items-center">
                <ActivityIndicator size="large" color="#7C3AED" />
                <Text className="mt-4 text-xl font-bold text-gray-900">Đang nộp bài...</Text>
              </View>
            ) : (
              <>
                ...
                {resultsAfterTest && (
                  <View className="bg-blue-50 rounded-2xl p-6 mb-6">
                    ...
                    <View className="items-center mb-4">
                      <Text className="text-gray-700 text-base mb-2">Điểm trung bình:</Text>
            ...
            >
              <Text
                className={`text-center text-base font-bold ${
                  recorded[currentQuestionIndex] ? 'text-white' : 'text-gray-400'
                }`}
              >
                {currentQuestionIndex < totalQuestions - 1 ? 'Tiếp tục →' : '✓ Hoàn thành bài thi'}
              </Text>
            </TouchableOpacity>
          </View>
      )}
